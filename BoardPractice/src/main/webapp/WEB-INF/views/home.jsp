...
<body>
	...
	<c:if test="${loginMember != null}">
	<p>
		<b>${loginMember.memberId}</b>님께서 로그인 중입니다. 
		...
		<tbody>
			<c:forEach items="${postList}" var="postList" begin="${postStartNum - 1}" end="${postLastNum - 1}">
				<tr>
					...
					<td>
						<a href="/readPost?postNum=${postList.postNum}">${postList.postTitle}</a>
						<c:if test="${postList.commentCount > 0}" >
							<span style="color:grey;">[${postList.commentCount}]</span>
						</c:if> 
					</td>
